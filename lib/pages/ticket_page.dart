          ...
          SnackBar(
            content: Row(
              children: [
                ...
                const Text(AppStrings.ticketSaved, style: TextStyle(fontWeight: FontWeight.w700)),
              ],
            ),
          ...
          child: Column(
            children: [
              AppBar(
                ...
                title: const Text(AppStrings.yourTicket, style: TextStyle(fontWeight: FontWeight.w900, letterSpacing: 3, fontSize: 14)),
                centerTitle: true,
              ),
                ...
                child: StreamBuilder<Ticket?>(
                  ...
                  builder: (context, snapshot) {
                    ...
                    if (currentTicket == null) {
                      return const Center(child: Text("Ticket introuvable", style: TextStyle(color: Colors.white)));
                    }
                    ...
                    return Center(
                      child: SingleChildScrollView(
                        padding: EdgeInsets.symmetric(horizontal: Responsive.padding, vertical: 24),
                        child: Column(
                          mainAxisAlignment: MainAxisAlignment.center,
                          children: [
                            RepaintBoundary(
                              key: _ticketKey,
                              child: _TicketCard(ticket: currentTicket, isCalled: isCalled),
                            ),
                                ...
                                child: Column(
                                  children: [
                                    ...
                                    Wrap(
                                      ...
                                      children: [
                                        _AlertChip(icon: Icons.check_circle, label: "Ticket confirmé", isActive: true),
                                        _AlertChip(icon: Icons.timer, label: "Alerte 30 min", isActive: false),
                                        _AlertChip(icon: Icons.alarm, label: "Alerte 10 min", isActive: false),
                                      ],
                                    ),
                                  ],
                                ),
                            ...
                            ElevatedButton.icon(
                              ...
                              label: const Text(AppStrings.downloadTicket, style: TextStyle(fontWeight: FontWeight.w900, letterSpacing: 1)),
                              style: ElevatedButton.styleFrom(
                                backgroundColor: Colors.white,
                                foregroundColor: AppColors.primary,
                                minimumSize: Size(double.infinity, Responsive.hp(7)),
                            ...
                            if (currentTicket.statut == 'valide' && currentTicket.rating == null)
                               _ReviewSection(ticketId: currentTicket.id).animate().fadeIn().scale(begin: const Offset(0.9, 0.9)),
                            const SizedBox(height: 16),
                            ElevatedButton(
                              onPressed: () {
                                HapticFeedback.mediumImpact();
                              ...
                              style: ElevatedButton.styleFrom(
                                ...
                                foregroundColor: Colors.white,
                                minimumSize: Size(double.infinity, Responsive.hp(8)),
                              ...
                              child: const Text(AppStrings.returnHome, style: TextStyle(fontWeight: FontWeight.w900, letterSpacing: 1)),
                            ),
                          ],
                        ),
                      ),
...
class _AlertChip extends StatelessWidget {
  ...
  const _AlertChip({required this.icon, required this.label, required this.isActive});
...
class _TicketCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
      ...
      child: Column(
        children: [
              ...
              children: [
                Container(
                  ...
                  child: Text(
                    ...
                    style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w900, color: AppColors.primary, letterSpacing: 2),
                  ),
                ),
                const SizedBox(height: 24),
                Text(
                ...
                Container(
                  ...
                  child: Column(
                    children: [
                      QrImageView(
                        data: 'SIRA-TICKET:${ticket.id}|${ticket.numeroTicket}|${ticket.clientNom}|${ticket.typeOperation}',
                ...
                if (isCalled)
                  const Column(
                    children: [
                      Icon(Icons.notifications_active_rounded, color: AppColors.statusOk, size: 56),
                      SizedBox(height: 16),
                      Text(
                        AppStrings.itsYourTurn,
                        style: TextStyle(fontSize: 26, fontWeight: FontWeight.w900, color: AppColors.statusOk, letterSpacing: -1),
                      ),
                      SizedBox(height: 8),
                      Text(
                        AppStrings.presentAtCounter,
                        style: TextStyle(fontSize: 14, color: AppColors.mutedForeground, fontWeight: FontWeight.w600),
                      ),
                    ],
                  ...
                  Column(
                    children: [
                      ...
                      const Text(
                        ...
                        style: TextStyle(fontSize: 13, color: AppColors.mutedForeground, fontWeight: FontWeight.w600),
                      ),
                    ],
                  ),
              ],
          ...
          Padding(
            ...
            child: Column(
              children: [
                _InfoRow(label: "CLIENT", value: ticket.clientNom.toUpperCase()),
                const SizedBox(height: 20),
                _InfoRow(label: "AGENCE", value: ticket.agenceNom.toUpperCase()),
                const SizedBox(height: 20),
    ...
    ).animate().fadeIn(duration: 800.ms).slideY(begin: 0.1, duration: 800.ms, curve: Curves.easeOutQuart);
  }
...
class _InfoRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Row(
      mainAxisAlignment: MainAxisAlignment.spaceBetween,
      children: [
        Text(label, style: const TextStyle(color: AppColors.mutedForeground, fontSize: 10, fontWeight: FontWeight.w900, letterSpacing: 1)),
        Text(value, style: const TextStyle(fontWeight: FontWeight.w900, fontSize: 13, color: AppColors.primary)),
      ],
    );
  }
}
              ...
              children: List.generate(5, (index) {
                return IconButton(
                  icon: Icon(
                    ...
                    color: index < _rating ? Colors.amber : Colors.white.withValues(alpha: 0.3),
                    size: 40,
                  ),
            ...
            if (_rating > 0) ...[
              ...
              ElevatedButton(
                onPressed: _isSubmitting ? null : () async {
                  ...
                  await _firebaseService.submitReview(widget.ticketId, _rating, _commentController.text);
                  if (mounted) {
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(content: Text(AppStrings.thanksFeedback)),
                    );
                  }
                },
                style: ElevatedButton.styleFrom(
                ...
                child: _isSubmitting 
                  ...
                  : const Text(AppStrings.send, style: TextStyle(fontWeight: FontWeight.w900, letterSpacing: 1)),
              ),
            ],
          ],
        ),
